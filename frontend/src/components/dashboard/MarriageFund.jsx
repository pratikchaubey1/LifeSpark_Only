...
export default function MarriageFund({ onMenuOpen }) {
    ...
    const loadAll = async () => {
        ...
        try {
            ...
            if (profileRes.ok) {
                setBalance(Number(profileData.user?.marriageFund) || 0);
                setUpiId(profileData.user?.upiId || "");
                setUpiNo(profileData.user?.upiNo || "");
            }

            if (listRes.ok) {
                // Filter for marriage fund withdrawals
                const allWithdrawals = Array.isArray(listData.withdrawals) ? listData.withdrawals : [];
                setWithdrawals(allWithdrawals.filter(w => w.source === 'marriageFund'));
            }
        } catch {
    ...
    const handleSubmit = async () => {
        ...
        if (amt > balance) {
            setMsgType("error");
            return setMsg("Insufficient marriage fund balance.");
        }
        if (!upiId.trim()) {
            setMsgType("error");
        ...
        try {
            setSubmitting(true);
            // Calls the specialized route for marriage fund
            const res = await fetch(`${API_BASE}/withdrawals/marriage-fund`, {
                method: "POST",
                headers: {
            ...
            setMsg("Marriage Fund withdrawal request submitted successfully.");
            setMsgType("success");
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50">
                ...
                <div className="mt-4 text-slate-500 font-medium">Loading your marriage fund...</div>
            </div>
        );
    }
            ...
            <div className="w-full max-w-5xl space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl md:text-2xl font-bold text-slate-900 leading-none flex items-center gap-2">
                                <FiHeart className="text-pink-500" /> Marriage Fund
                            </h2>
                            <div className="text-xs text-slate-500 mt-1">Manage your marriage support fund</div>
                        </div>
                    </div>

                    <button
                        onClick={loadAll}
                        className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-pink-50 text-pink-600 hover:bg-pink-100 transition active:scale-95"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-12 xl:col-span-5 space-y-6">
                        ...
                        {/* BALANCE CARD */}
                        <div className="bg-pink-600 rounded-3xl p-8 text-white shadow-xl shadow-pink-200 relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-8 opacity-10">
                                <FiHeart size={120} />
                            </div>
                            <div className="relative z-10">
                                <div className="text-pink-100 text-sm font-medium uppercase tracking-wider">Available Fund</div>
                        ...
                        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden h-full flex flex-col">
                            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                                <h3 className="font-bold text-slate-900 flex items-center gap-2">
                                    <FiClock className="text-pink-600" /> Fund History
                                </h3>
                            </div>
...
function InputGroup({ icon, label, placeholder, value, onChange, type = "text" }) {
    return (
        <div className="space-y-1.5 focus-within:text-pink-600 transition-colors">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">{label}</label>
            <div className="group relative flex items-center">
                <div className="absolute left-4 text-slate-400 group-focus-within:text-pink-600 transition-colors">
                    {icon}
                </div>
                <input
                    type={type}
                    ...
                    className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium outline-none focus:bg-white focus:border-pink-600 focus:ring-4 focus:ring-pink-50 shadow-sm transition-all text-slate-700"
                />
            </div>
        </div>
    );
